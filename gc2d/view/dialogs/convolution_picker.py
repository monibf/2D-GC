...
class ConvolutionPicker(QDialog):
    ...
    def __init__(self, model_wrapper):
        ...
        # No transform
        self.add_button(Transform, "No Transform",
                        "No transformation is linked under 'show transformed data' with this option.", [], checked=True)

        # Static cut-off
        self.add_button(StaticCutoff, "Static cut-off", "From each point in the graph, the given value is subtracted.",
                        [_ParamDouble("cut-off value: ", value=self.model_wrapper.model.upper_bound / 10)])
        ...
        self.add_button(Gaussian, "Gaussian Convolution", "Convolves the data with a gaussian kernel",
                        [_ParamDouble("Sigma: ")])
        ...
        self.add_button(
            ...
            [_ParamCSV("kernel file: ", "CSV Files (*.csv);;All Files(*)", "open kernel",
                       "open convolution kernel CSV")]
        )
